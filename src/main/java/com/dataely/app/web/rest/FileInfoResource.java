...
public class FileInfoResource {
    ...
    private static final String ENTITY_NAME = "dataelyAppFileInfo";
    ...
    public ResponseEntity<FileInfo> createFileInfo(@Valid @RequestBody FileInfo fileInfo) throws URISyntaxException {
     ...
     *
     * @param id the id of the fileInfo to save.
    ...
    public ResponseEntity<FileInfo> updateFileInfo(
        ...
        log.debug("REST request to update FileInfo : {}, {}", id, fileInfo);
        if (fileInfo.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, fileInfo.getId())) {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, fileInfo.getId().toString()))
            .body(result);
    }
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated fileInfo,
     * or with status {@code 400 (Bad Request)} if the fileInfo is not valid,
     * or with status {@code 404 (Not Found)} if the fileInfo is not found,
     * or with status {@code 500 (Internal Server Error)} if the fileInfo couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/file-infos/{id}", consumes = "application/merge-patch+json")
    public ResponseEntity<FileInfo> partialUpdateFileInfo(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody FileInfo fileInfo
    ) throws URISyntaxException {
        log.debug("REST request to partial update FileInfo partially : {}, {}", id, fileInfo);
        if (fileInfo.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, fileInfo.getId())) {
        ...
        Optional<FileInfo> result = fileInfoService.partialUpdate(fileInfo);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, fileInfo.getId().toString())
        );
    }
    ...
    public ResponseEntity<List<FileInfo>> getAllFileInfos(Pageable pageable) {
        log.debug("REST request to get a page of FileInfos");
        Page<FileInfo> page = fileInfoService.findAll(pageable);
    ...
    public ResponseEntity<FileInfo> getFileInfo(@PathVariable Long id) {
        ...
        Optional<FileInfo> fileInfo = fileInfoService.findOne(id);
